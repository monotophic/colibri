 ...
 * LOADER's own disambiguation/stamp logic in isolation, but the container
 * bytes are hand-authored, not tool-produced) nor test_fp8_repack.py (proves
 * the WRITER's Python-side output shape -- never invokes the C loader at
 * all) exercises the REAL round trip: a container the REAL
 ...
 *   (a) fmt==100 resolved (native fp8-e4m3-passthrough -- byte-arithmetic
...
int main(int argc, char **argv){
    ...
    for(int i=0;i<ntensors;i++){
        ...
        if(t.fmt != 100){
            printf("FAIL %s: fmt=%d, expected 100 (native fp8-e4m3 passthrough)\n", name, t.fmt);
            fails++; continue;
        }
        if(!t.q8 || !t.s){
            printf("FAIL %s: fmt=100 but q8=%p s=%p (expected both non-NULL)\n",
        ...
        printf("ok %s: fmt=100 O=%d I=%d, weights+scale loaded through the real loader, all-finite\n",
               name, O, I);
    }
